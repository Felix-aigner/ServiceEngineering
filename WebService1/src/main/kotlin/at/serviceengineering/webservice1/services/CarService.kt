...
class CarService(
    ...
    override fun findAll(currency: Currency): List<CarDto> = carRepository.findAll().map { car: Car ->
        carMapper.mapToCarDtoWithCustomCurrency(car, currency)
    }
    ...
    override fun deleteCar(id: UUID) {
        rentalService.findAllByCarId(id)
                .forEach {
                    it.car = null
                    rentalService.saveEntity(it)
                }
